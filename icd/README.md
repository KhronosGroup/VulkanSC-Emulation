...
The behavior of the Vulkan SC Emulation ICD can be controlled with the following environment variables:
  ...
  * `VKSC_EMULATION_MAX_LOGICAL_DEVICES` can be used to set the maximum number of logical devices that can be createad for a single physical device. The default limit is `65536`. Setting this to `1` can be used to emulate implementations that only support creating a single `VkDevice` in a single `VkInstance`. Note that this limit applies per `VkInstance`, therefore creating more logical devices across multiple `VkInstance` objects is still allowed.
