...
class UUID {
  public:
    struct random_t {};
    static constexpr random_t random{};
    ...
    UUID(random_t tag, const char* json_str) {
        const auto nanoseconds_since_epoch =
            std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
        const std::uint8_t variant = 0b000;
        const std::uint8_t version = 0b10000;
        static_assert(sizeof(std::random_device::result_type) >= sizeof(std::uint16_t),
                      "Impl assumes std::random_device::result_type to be at least 2 bytes.");
        const auto clock_sequence = std::random_device{}();

        const std::uint32_t time_low = static_cast<std::uint32_t>(nanoseconds_since_epoch);
        const std::uint16_t time_mid = static_cast<std::uint16_t>(nanoseconds_since_epoch >> sizeof(time_low));
        const std::uint16_t time_hi_and_version =
            (static_cast<std::uint16_t>(nanoseconds_since_epoch >> sizeof(time_low + time_mid)) & 0xafff) |
            (static_cast<std::uint16_t>(version) << 12);
        const std::uint8_t clock_seq_hi_and_reserved =
            (static_cast<std::uint8_t>(clock_sequence >> 8) & 0b00011111) | (variant << 5);
        const std::uint8_t clock_seq_low = static_cast<std::uint8_t>(clock_sequence);
        const std::size_t node = std::hash<std::string>{}(json_str);
